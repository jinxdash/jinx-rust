import { is_LocArray, is_Node } from "./ast";
import { clamp, color, getTerminalWidth, normPath, reduce_tagged_template } from "./common";
...
export function d(...args: [strings: TemplateStringsArray, ...values: (string | number | { loc: object } | Function)[]]) {
	return reduce_tagged_template(args, (data) =>
		null == data
			? color.magenta("" + data)
			: is_Node(data)
			? "name" in data
				? TY(REF(data.type), VAL(`"${data.name}"`)) // Identifier<"foo">
				: "token" in data
				? TY(REF(data.type), VAL(`"${data.token}"`)) // Token<"=">
				: REF(data.type)
			: is_LocArray(data)
			? REF("LocArray")
	...
	function TY(thing: string, data: string) {
		return color.grey(`${thing}<${data}>`);
	}
	function VAL(data: any) {
...
function composeCentered(str: string, len = 64) {
	const space = " ".repeat((clamp(0, len, len - color.unstyledLength(str)) / 2) | 0);
